...
public class SimpleCA_Centre {
    ...
    public static void main(String args[]) throws Exception {
        ...
        // Define initial state - excited in the centre. 
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                state[i][j] = 0;
            }
        ...
        while (true) {
            ...
            // Chop wave when half-way up. As set to chop the left part, we will see two sprial waves, one at top and one at bottom, and they will collide at the middle of the grid. 
            if (iter == N / 2) {
                for (int i = 0; i < N / 2; i++) {
                    for (int j = 0; j < N/2; j++) {
                        state[i][j] = 0;
